...
/**
 * Simple Swing-based timer that periodically notifies registered {@link IPing}
 * listeners.
 * <p>
 * Internally this class uses a {@link javax.swing.Timer}, so all callbacks
 * occur on the Swing Event Dispatch Thread (EDT). This makes it suitable for
 * driving lightweight UI updates, animation steps, or regular housekeeping work
...
public class Ping implements AutoCloseable {
	...
	/**
	 * Creates a new {@code Ping} that fires at the specified interval and starts
	 * immediately.
	 *
	 * @param delayInMillis the delay between pings in milliseconds; must be
	 *                      positive
	 *
	 ...
	 * @param delayInMillis    the delay between pings in milliseconds; must be
	 *                         positive
	 * @param startImmediately if {@code true}, the timer is started as soon as the
	 *                         {@code Ping} is created; otherwise it remains stopped
	 ...
	 * <p>
	 * All subsequent ticks will result in calls to {@link IPing#ping()} for each
	 * registered listener until {@link #stop()} or {@link #close()} is called.
	 * </p>
	 */
	public void start() {
		if (!timer.isRunning()) {
	...
	/**
	 * Stops the underlying timer. No further {@link IPing#ping()} callbacks will
	 * occur until {@link #start()} is called again.
	 */
	public void stop() {
		if (timer.isRunning()) {
			timer.stop();
	...
	/**
	 * Notifies all registered {@link IPing} listeners that a timer tick has
	 * occurred.
	 * <p>
	 * This method is invoked on the Swing Event Dispatch Thread (EDT) by the
	 * internal {@link javax.swing.Timer}.
	 * </p>
	 */
	 ...
	 * <p>
	 * This method simply calls {@link #stop()} and is provided so that {@code Ping}
