...
public class FeedbackPane extends TextPaneScrollPane {
	 ...
	 * send the message. To use a different x11 color, prepend the message with the
	 * x11 name embedded between $ characters. For example<br>
		...
		if (message.startsWith("$")) {
			...
			if ((nextIndex > 3) && (nextIndex < 30)) {
				...
				if (style == null) {
					...
					if (color != null) {
						style = createStyle(color, _background, "SansSerif", _fontSize, false, true);
