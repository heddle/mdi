...
public final class ProjectionFactory {
     ...
     * Conventions:
     * <ul>
     *   <li>Geographic center is represented by a {@link Point2D.Double}
     *       where {@code x = λ} (longitude, radians) and
     *       {@code y = φ} (latitude, radians).</li>
     *   <li>If {@code center} is {@code null}, a projection-specific default
     *       center is used.</li>
     *   <li>Projections that do not support a center will ignore the
     *       {@code center} value.</li>
     * </ul>
     *
     * @param type    the projection type (must not be {@code null})
     * @param theme   the map theme to use (must not be {@code null})
     * @param center  optional projection center in radians; {@code null} allows
     *                the factory to choose a sensible default
     * @return a new {@link IMapProjection} instance
