 ...
 * points (and optional labels) using an {@link IMapProjection},
 * {@link MapTheme}, and {@link IContainer}.
 * <p>
 * The renderer uses the projection's {@link IMapProjection#getXYBounds()} to
 * construct a world-to-screen transform via the container, so that multiple
 * layers (countries, graticule, cities) line up.
 * </p>
 * <p>
 * Conventions:
 * <ul>
 ...
 * <li>They are stored in a {@link Point2D.Double} with {@code x = λ}
 * (longitude) and {@code y = φ} (latitude).</li>
...
public class CityPointRenderer {
	...
	/**
	 * Maximum scalerank to draw; negative means no scalerank filter. If set to 0
	 * only the most important cities are drawn.
	 */
	 ...
	 * <p>
	 * By default this is set to a modest value, so only relatively important cities
	 * receive labels, even if smaller towns still get dots.
	 * </p>
	 */
	 ...
	 */
	public CityPointRenderer(List<GeoJsonCityLoader.CityFeature> cities, IMapProjection projection) {
	...
	/**
	 * Only draw cities with {@code scalerank <= maxScalerank}. Set to a negative
	 * value to disable this filter entirely.
	 */
	 ...
	 * filtering. A negative value disables additional label filtering, so any city
	 * that is drawn can be labeled (subject to {@link #setDrawLabels}).
	 *
	 * @param maxLabelScalerank maximum scalerank allowed for labels, or a negative
	 *                          value to disable label filtering by scalerank.
	 */
	...
	/**
	 * Return whether the city passes the population and global scalerank filters
	 * used for drawing the point itself.
	 */
	private boolean passesFilters(GeoJsonCityLoader.CityFeature city) {
		if (minPopulation > 0 && city.getPopulation() >= 0 && city.getPopulation() < minPopulation) {
			return false;
		}
		if (maxScalerank >= 0 && city.getScalerank() >= 0 && city.getScalerank() > maxScalerank) {
			return false;
		}
		return true;
	}
	...
	/**
	 * Return whether the city passes the more restrictive label scalerank filter
	 * (if enabled). This is evaluated only after the city has already passed
	 * {@link #passesFilters(GeoJsonCityLoader.CityFeature)}.
	 */
	 ...
	 * @return closest city within a small picking radius, or {@code null} if none
	 *         are close enough.
	 */
