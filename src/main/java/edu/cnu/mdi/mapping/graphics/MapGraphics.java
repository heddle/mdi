...
/**
 * Static utility methods for drawing and geometry operations on geographic data
 * in the MDI mapping subsystem.
 *
 * <h2>Purpose</h2>
 * <p>
 ...
 * <li>Forward projection from geographic coordinates (longitude, latitude) to
 * projection-space (x, y) coordinates via {@link IMapProjection}.</li>
 * <li>Seam-aware splitting of projected paths so that segments crossing a
 * longitude wrap boundary are not drawn as spurious streaks across the full
 * map.</li>
 * <li>Conversion from projection-space coordinates to device pixels through the
 * owning {@link MapContainer}.</li>
 * </ol>
 *
...
public final class MapGraphics {
	 ...
	 *
	 * <p>
	 * Smaller values produce smoother curves and more accurate seam placement at
	 * the cost of more projected vertices.
	 * </p>
	 */
	 ...
	 * @param container the map container that owns the projection and supplies the
	 ...
	 */
	public static void drawMapLine(Graphics2D g2, MapContainer container, Point2D.Double ll0, Point2D.Double ll1,
		...
		drawMapPolyline(g2, container, new Point2D.Double[] { ll0, ll1 }, lineColor, lineWidth, lineStyle);
	}
	 ...
	 * @param latLonPoints geographic vertices in radians; at least two points are
	 *                     required
	 * @param lineColor    the line color; {@code null} suppresses drawing
	 * @param lineWidth    the line width in pixels
	 * @param lineStyle    the line dash style; if {@code null},
	 *                     {@link LineStyle#SOLID} is used
	 */
	public static void drawMapPolyline(Graphics2D g2, MapContainer container, Point2D.Double[] latLonPoints,
			Color lineColor, float lineWidth, LineStyle lineStyle) {
		...
		ProjectedMapShape shape = buildProjectedPolyline(container, latLonPoints, DEFAULT_MAX_GREAT_CIRCLE_STEP_DEG);
	 ...
	 *
	 * <p>
	 * The polygon is automatically closed by connecting the last vertex back to the
	 * first using a great-circle segment.
	 * </p>
	 *
	 * @param g2           the graphics context
	 * @param container    the owning map container
	 * @param latLonPoints geographic polygon vertices in radians; at least three
	 *                     points are required
	 ...
	 *                     {@link LineStyle#SOLID} is used
	 */
	public static void drawMapPolygon(Graphics2D g2, MapContainer container, Point2D.Double[] latLonPoints,
			Color fillColor, Color lineColor, float lineWidth, LineStyle lineStyle) {

		ProjectedMapShape shape = buildProjectedPolygon(container, latLonPoints, DEFAULT_MAX_GREAT_CIRCLE_STEP_DEG);
	...
	public static void drawMapPolyline(Graphics2D g2, MapContainer container, Point2D.Double[] latLonPoints,
		...
		drawMapPolyline(g2, container, latLonPoints, style.getLineColor(), style.getLineWidth(), style.getLineStyle());
	}
	...
	public static void drawMapPolygon(Graphics2D g2, MapContainer container, Point2D.Double[] latLonPoints,
		...
		drawMapPolygon(g2, container, latLonPoints, style.getFillColor(), style.getLineColor(), style.getLineWidth(),
				style.getLineStyle());
	}
	 ...
	 */
	public static ProjectedMapShape buildProjectedPolyline(MapContainer container, Point2D.Double[] latLonPoints) {
		return buildProjectedPolyline(container, latLonPoints, DEFAULT_MAX_GREAT_CIRCLE_STEP_DEG);
	}
	 ...
	 * @return a seam-split projected shape; never {@code null}
	 */
	public static ProjectedMapShape buildProjectedPolygon(MapContainer container, Point2D.Double[] latLonPoints) {
		return buildProjectedPolygon(container, latLonPoints, DEFAULT_MAX_GREAT_CIRCLE_STEP_DEG);
	}
	 ...
	 *                     great-circle segment; values less than or equal to zero
	 *                     are replaced by
	 ...
	 * @return the union of all path bounds, or an empty rectangle if the shape is
	 *         {@code null} or empty
	 */
	...
	/**
	 * Tests whether a device-space point lies inside any closed projected subpath
	 * of a map shape.
	 *
	 * <p>
	 ...
	 * @return {@code true} if any closed subpath contains the point; {@code false}
	 *         otherwise
	 */
	...
	/**
	 * Tests whether a device-space point lies within a specified tolerance of any
	 * projected path segment.
	 *
	 * <p>
	 * This method is useful for hit testing open polylines and polygon outlines.
	 * </p>
	 *
	 * @param shape  the projected map shape
	 * @param p      the test point in device coordinates
	 * @param tolPix the tolerance in pixels; values less than zero are treated as
	 *               zero
	 * @return {@code true} if the point is within {@code tolPix} of any segment;
	 *         {@code false} otherwise
	 */
	public static boolean isPointNear(ProjectedMapShape shape, Point p, double tolPix) {
	...
	/**
	 * Normalizes a geographic point by wrapping its longitude to the projection's
	 * canonical range and clamping latitude to [-π/2, π/2].
	 *
	 * @param projection the active map projection; must not be {@code null}
	 * @param latLon     the geographic point in radians; must not be {@code null}
	 *                   The x coordinate is interpreted as longitude and the y
	 *                   coordinate as latitude.
	 * @return a normalized copy of the input point
	 */
	public static Point2D.Double normalizeLatLon(IMapProjection projection, Point2D.Double latLon) {
		...
		double lat = Math.max(-0.5 * Math.PI, Math.min(0.5 * Math.PI, latLon.y));
	 ...
	 * @param lineStyle outline dash style; if {@code null}, {@link LineStyle#SOLID}
	 *                  is used
	 */
	private static void drawProjectedShape(Graphics2D g2, ProjectedMapShape shape, Color fillColor, Color lineColor,
		...
		if (lineColor != null) {
			...
			g2.setStroke(GraphicsUtils.getStroke(Math.max(MIN_LINE_WIDTH, lineWidth),
	 ...
	 * @param closed       whether the last vertex should connect back to the first
	 * @param maxStepDeg   maximum great-circle sampling step in degrees
	 * @return a seam-aware projected shape; never {@code null}
	 */
	private static ProjectedMapShape buildProjectedShape(MapContainer container, Point2D.Double[] latLonPoints,
			boolean closed, double maxStepDeg) {
		...
		double stepDeg = (maxStepDeg > 0.0) ? maxStepDeg : DEFAULT_MAX_GREAT_CIRCLE_STEP_DEG;
		...
		for (int i = 0; i < segmentCount; i++) {
			...
			Point2D.Double ll1 = normalizeLatLon(projection, latLonPoints[(i + 1) % n]);
			...
			/*
			 * Reset continuity between source segments. This avoids a false connection
			 * between the end of one sampled segment and the start of the next when they
			 * happen to begin after a seam break or an invisible region.
			 */
//            prevSample = null;
//            prevVisible = false;
//            prevPixel = null;
//            currentPath = null;
	...
	private static IMapProjection getProjection(MapContainer container) {
		if (!(container.getView() instanceof MapView2D mapView)) {
			throw new IllegalStateException("MapContainer is not attached to a MapView2D");
		}
	 ...
	 */
	private static Point2D.Double projectLatLon(IMapProjection projection, Point2D.Double latLon) {
	 ...
	 */
	private static boolean isProjectedPointDrawable(IMapProjection projection, Point2D.Double latLon,
			Point2D.Double xy) {
	 ...
	 */
	private static Point projectedToPixel(MapContainer container, Point2D.Double xy) {
	 ...
	 * @return {@code true} if both points are non-null and have identical integer
	 *         coordinates
	 */
	 ...
	 *
	 * <p>
	 * The result includes both endpoints. Sampling is performed by spherical linear
	 * interpolation (slerp) on the unit sphere, then converting each interpolated
	 * Cartesian point back to longitude/latitude.
	 * </p>
	 *
	 * <p>
	 * If the endpoints are nearly identical, a two-point list containing the
	 * normalized endpoints is returned. If the endpoints are nearly antipodal, the
	 * interpolation still proceeds, but note that the great circle is then not
	 * unique. In that rare case the result depends on floating-point roundoff in
	 * the two endpoint vectors.
	 * </p>
	 *
	 ...
	 */
	private static List<Point2D.Double> sampleGreatCircle(Point2D.Double ll0, Point2D.Double ll1, double maxStepDeg) {
		...
		double step = (maxStepDeg > 0.0) ? maxStepDeg : DEFAULT_MAX_GREAT_CIRCLE_STEP_DEG;
		int nstep = Math.max(1, (int) Math.ceil(Math.toDegrees(omega) / step));
	 ...
	 * @return {@code true} if the point lies within tolerance of any path segment;
	 *         {@code false} otherwise
	 */
	private static boolean isPointNearPath(Path2D.Double path, Point p, double tol2) {
		...
		while (!pi.isDone()) {
			...
			default -> throw new IllegalArgumentException("Path contains unsupported curve segments");
			}
	...
	/**
	 * Returns the squared distance from a point to a line segment in device-space.
	 *
	 ...
	 * crosses the seam of a wrapped map projection. Each path is already expressed
	 * in device coordinates and is therefore ready for drawing and hit testing.
	 * </p>
	 */
	...
	/**
	 * Computes the central-angle length of the shorter great-circle arc between two
	 * geographic points on the unit sphere.
	 *
	 * <p>
	 * The input points use the standard MDI geographic convention:
	 * {@code x = longitude} and {@code y = latitude}, both in radians. The returned
	 * value is also in radians. On a sphere of radius {@code R}, the physical arc
	 * length is therefore {@code R * greatCircleLength(...)}.
	 * </p>
	 *
	 * <p>
	 * The computation uses the spherical law of cosines in a numerically safe form
	 * by clamping the cosine argument to {@code [-1, 1]} before calling
	 ...
	 *                    ({@code x = longitude, y = latitude}); must not be
	 *                    {@code null}
	 * @return the shorter great-circle arc length in radians on the unit sphere, in
	 *         the range {@code [0, π]}
	 * @throws IllegalArgumentException if either argument is {@code null}
	 */
	public static double greatCircleLength(Point2D.Double startLatLon, Point2D.Double endLatLon) {
	...
	/**
	 * Computes the initial great-circle azimuth (bearing) from a start geographic
	 * point to an end geographic point on the unit sphere.
	 *
	 * <p>
	 * The input points use the standard MDI geographic convention:
	 * {@code x = longitude} and {@code y = latitude}, both in radians. The returned
	 * azimuth is also in radians and uses the navigation convention:
	 * </p>
	 * <ul>
	 * <li>{@code 0} = north</li>
	 * <li>{@code π/2} = east</li>
	 ...
	 *
	 * <p>
	 * The value is normalized to the half-open range {@code [0, 2π)}. This is the
	 * <em>initial</em> azimuth at the start point. Along a great-circle path the
	 * local heading generally changes continuously, so the azimuth at the end point
	 ...
	 * @throws IllegalArgumentException if either argument is {@code null}
	 */
	public static double greatCircleAzimuth(Point2D.Double startLatLon, Point2D.Double endLatLon) {
	...
	/**
	 * Computes the endpoint of a great-circle path on the unit sphere from a start
	 * geographic point, an initial azimuth, and a normalized arc length.
	 *
	 * <p>
	 * The input start point uses the standard MDI geographic convention:
	 * {@code x = longitude} and {@code y = latitude}, both in radians. The azimuth
	 * is also in radians and uses the navigation convention:
	 * </p>
	 * <ul>
	 * <li>{@code 0} = north</li>
	 * <li>{@code π/2} = east</li>
	 ...
	 *
	 * <p>
	 * The {@code normalizedLength} parameter is the angular arc length on the unit
	 * sphere, in radians. Thus:
	 * </p>
	 * <ul>
	 ...
	 *
	 * <p>
	 * The returned longitude is wrapped to {@code (-π, π]} using the same canonical
	 * convention used elsewhere in the mapping subsystem. The returned latitude is
	 * naturally in {@code [-π/2, π/2]}.
	 * </p>
	 *
	 * @param startPoint       the start geographic point in radians
	 *                         ({@code x = longitude, y = latitude}); must not be
	 *                         {@code null}
	 * @param azimuth          the initial azimuth in radians, using the navigation
	 *                         convention
	 * @param normalizedLength the angular arc length on the unit sphere in radians
	 ...
	 */
	public static Point2D.Double greatCircleEndPoint(Point2D.Double startPoint, double azimuth,
	...
	/**
	 * Small immutable 3D vector used internally for great-circle interpolation on
	 * the unit sphere.
	 */
	private static final class Vec3 {
		...
		/**
		 * Builds a unit-sphere Cartesian vector from a geographic point in radians.
		 *
		...
		static Vec3 fromLatLon(Point2D.Double latLon) {
			...
			return new Vec3(cosLat * Math.cos(lon), cosLat * Math.sin(lon), Math.sin(lat));
		}
	...
	/**
	 * Draws a horizontal latitude line across the map at a specified latitude.
	 *
	 * <p>
	 * The line is drawn as a single straight segment between the left and right
	 * edges of the map, which is appropriate for projections with parallels are
	 * straight horizontal lines (e.g., cylindrical projections).
	 * </p>
	 *
	 * @param g2         the graphics context
	 * @param container  the owning map container
	 * @param latitude   the latitude in radians at which to draw the line
	 * @param centralLon the central longitude in radians used to determine the left
	 *                   and right edges of the map; typically this is the current
	...
	public static void drawHorizontalLatitudeLine(Graphics2D g2, 
		...
		Point2D.Double xy = new Point2D.Double();
		latLon.y = latitude;
		double startLon = projection.wrapLongitude(centralLon - Math.PI + EPSILON);
		double endLon = projection.wrapLongitude(centralLon + Math.PI - EPSILON);

		latLon.x = startLon;
		projection.latLonToXY(latLon, xy);
		container.worldToLocal(pp0, xy);

		latLon.x = endLon;
		projection.latLonToXY(latLon, xy);
		container.worldToLocal(pp1, xy);
	...
	/**
	 * Draws a vertical longitude line across the map at a specified longitude.
	 *
	 * <p>
	 * The line is drawn as a single straight segment between the top and bottom
	 * edges of the map, which is appropriate for projections with meridians are
	 * straight vertical lines (e.g., cylindrical projections).
	 * </p>
	 *
	 * @param g2        the graphics context
	 * @param container the owning map container
	 * @param longitude the longitude in radians at which to draw the line
	 * @param theme     the map theme providing styling information; must not be
	 *                  {@code null}
	 */
	public static void drawVerticalLongitudeLine(Graphics2D g2, 
		...
		Point2D.Double xy = new Point2D.Double();
		latLon.x = longitude;
		double startLat = -0.5 * Math.PI + EPSILON;
		double endLat = 0.5 * Math.PI - EPSILON;
