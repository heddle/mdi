 ...
 * <h2>Domain</h2>
 * <p>
 * The entire sphere maps to a disk of radius 2 (for R = 1). The antipode of the
 * center point maps to the outer boundary circle. The bounding box is [-2, -2]
 * to [2, 2].
 * </p>
 *
 * <h2>Antipode singularity</h2>
 * <p>
 * When the denominator {@code 1 + cos γ} approaches zero, the input point is at
 * or extremely close to the antipode of the projection center. The antipode is
 * not represented by one unique point in the projected disk; the limiting point
 * on the boundary depends on the direction of approach. This implementation
 * therefore returns {@code NaN} for the exact singular point, and drawing
 * routines break the polyline at that location.
 * </p>
 *
 * <h2>No seam</h2>
 * <p>
 * The Lambert projection has no antimeridian seam because the mapping from
 * globe to disk is continuous. The default {@link IMapProjection#crossesSeam}
 * returning {@code false} is correct for this class.
 * </p>
 */
public class LambertEqualAreaProjection implements IMapProjection {
	...
	/**
	 * Maximum radial distance in projection space (antipode of center maps to the
	 * circle boundary at ρ = 2R).
	 */
	 ...
	 * before any drawing methods are called, or use the three-argument constructor
	 * to set it atomically.
	 *
	...
	/**
	 * Convenience constructor centered at (0°, 0°). The theme must still be set
	 * before drawing.
	 */
	...
	/**
	 * Convenience constructor that sets both the center and the theme atomically,
	 * preventing the NullPointerException that arises when drawing methods are
	 * called before the theme is set.
	 *
	 * @param centerLon central longitude λ₀ in radians
	 * @param centerLat central latitude φ₀ in radians
	...
	/**
	 * Changes the center of the projection. The next repaint will show the globe
	 * centred on the new point.
	 *
	 * @param centerLon central longitude λ₀ in radians
	 * @param centerLat central latitude φ₀ in radians
	 */
	...
	public void latLonToXY(Point2D.Double latLon, Point2D.Double xy) {
		...
		xy.setLocation(k * cosLat * Math.sin(dLon), k * (cosLat0 * sinLat - sinLat0 * cosLat * cosDLon));
	}
	...
	public void latLonFromXY(Point2D.Double latLon, Point2D.Double xy) {
		...
		latLon.x = wrapLongitude(centerLon + Math.atan2(x * sinC, rho * cosLat0 * cosC - y * sinLat0 * sinC));
	}
	 ...
	 *
	 * <p>
	 * The Lambert projection maps the entire globe, so every point in the valid
	 * latitude range is visible.
	 * </p>
	 */
	@Override
	 ...
	 *
	 * <p>
	 * A point is on the map when it lies within or on the boundary of the disk: x²
	 * + y² ≤ RHO_MAX².
	 * </p>
	 */
	@Override
	 ...
	 *
	 * <p>
	 * Draws the circular boundary of the projection disk (radius
	 * {@code RHO_MAX = 2}) using the theme's outline color and stroke width.
	 * </p>
	 */
	@Override
	 ...
	 *
	 * <p>
	 * Samples 360 evenly-spaced longitudes at the given latitude and connects the
	 * projected screen points. NaN results (e.g. near the antipode boundary) are
	 * skipped. Uses {@link MapTheme#getGraticuleColor()} so the line color reflects
	 * the active theme.
	 * </p>
	 */
	@Override
	public void drawLatitudeLine(Graphics2D g2, IContainer container, double latitude) {
		...
		latLon.y = lat;
		for (int i = 0; i <= 360; i++) {
			latLon.x = -Math.PI + 2.0 * Math.PI * i / 360.0;
	 ...
	 *
	 * <p>
	 * Samples 180 evenly-spaced latitudes at the given longitude and connects the
	 * projected screen points. Uses {@link MapTheme#getGraticuleColor()} for the
	 * line color.
	 * </p>
	 */
	@Override
	 ...
	 *
	 * <p>
	 * Returns the bounding square of the disk: [-RHO_MAX, -RHO_MAX] to [RHO_MAX,
	 * RHO_MAX], i.e. [-2, -2] to [2, 2] for a unit sphere.
	 * </p>
	 */
	@Override
	 ...
	 *
	 * <p>
	 * Returns a circular path in device coordinates matching the disk boundary
	 * (radius {@code RHO_MAX}), used as a clip region for ocean fill and land
	 * rendering.
	 * </p>
	 */
	@Override
