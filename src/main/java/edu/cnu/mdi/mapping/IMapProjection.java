 ...
 * where {@code x = λ} (longitude, in radians) and {@code y = φ} (latitude, in
 * radians).</li>
 * <li>Projected (map) coordinates are also represented by
 * {@link java.awt.geom.Point2D.Double} and are referred to as "world" or
 * "projection-space" coordinates. The {@link IContainer} is responsible for
 * converting world coordinates to device coordinates (pixels).</li>
 * </ul>
 */
public interface IMapProjection {
	 ...
	 *
	 * @param latLon output geographic point (in radians). {@code x = λ},
	 *               {@code y = φ}; must not be {@code null}
	...
	/**
	 * Draw the outline of the map supported by this projection using the provided
	 * graphics context and container. Implementations typically:
	 * <ol>
	 * <li>Construct a {@link java.awt.geom.Path2D} in world coordinates describing
	 * the boundary of the valid projected domain.</li>
	 * <li>Transform those points to screen space using the container's
	 * world-to-local transform.</li>
	 * <li>Stroke the resulting path using colors and strokes from the current
	 * {@link MapTheme}.</li>
	 * </ol>
	 *
	...
	/**
	 * Test whether a projection-space point lies within the valid domain of this
	 * projection.
	 *
	 ...
	 * Implementations typically approximate the parallel by sampling in longitude,
	 * projecting each sample, and connecting them with a polyline.
	 *
	 ...
	 * Implementations typically approximate the meridian by sampling in latitude,
	 * projecting each sample, and connecting them with a polyline.
	 *
	 * @param g2        graphics context
	 * @param container container providing world-to-local transforms
	 ...
	 * <ul>
	 * <li>Global projections (e.g. Mercator, Mollweide) usually return {@code true}
	 * for all valid lat/lon values.</li>
	...
	/**
	 * Human-readable name for this projection. The default implementation delegates
	 * to {@link EProjection#getName()} if available, otherwise falls back to the
	 * simple class name.
	 *
	...
	/**
	 * Bounding box of the projection-space domain, in world coordinates. This
	 * rectangle is typically used by containers to build a world-to-screen
	 * transform.
	 *
	...
	/**
	 * Set the {@link MapTheme} used by this projection for outlines and graticule
	 * drawing.
	 *
	...
	/**
	 * Create a clip shape in <em>device coordinates</em> corresponding to the map's
	 * valid domain. This is typically used to:
	 * <ul>
	 * <li>Clip land/ocean rendering to the map region.</li>
	 * <li>Fill the ocean background via
	 * {@link #fillOcean(Graphics2D, IContainer)}.</li>
	 * </ul>
	 *
	...
	/**
	 * Convenience helper that fills the map domain with the ocean color from the
	 * current {@link MapTheme}.
	 *
	 * @param g2        graphics context
	 * @param container container providing world-to-local transforms
	 */
	...
	/**
	 * Wrap a latitude value to the range [-π/2, π/2]. This is mainly useful for
	 * helper code; most projections should clamp, not wrap, latitude.
	 *
