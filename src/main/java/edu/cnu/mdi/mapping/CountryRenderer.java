 ...
 * {@link MapTheme}.
 * <p>
 * This renderer uses a {@link CountryShapeCache} to store the projected country
 * polygons in world (projection XY) coordinates. At render time, the cached
 * world paths are transformed to screen space using the container's
 * {@code worldToLocal} transform so that zoom and pan are automatically
 * respected.
 * </p>
 * <p>
...
public class CountryRenderer {
	...
	/**
	 * Construct a new renderer for the given country features and projection. The
	 * current theme is obtained from the supplied projection.
	 *
	 ...
	 *
	 * @param fillLand {@code true} to fill land areas; {@code false} to draw only
	 *                 borders
	 */
	 ...
	 *
	 * @param drawBorders {@code true} to draw borders; {@code false} to skip border
	 *                    strokes
	 */
	 ...
	 *
	 * @param useAntialias {@code true} to enable antialiasing; {@code false} to use
	 *                     the existing hint
	 */
	...
	/**
	 * Draw a single cached country shape. The shape is stored in world coordinates;
	 * this method converts it to a screen-space path using the container's
	 * {@code worldToLocal} transform.
	 *
	...
	/**
	 * Perform a hit-test on the cached country shapes to determine which country
	 * (if any) lies under the given mouse position.
	 *
	 ...
	 * @return the first {@link GeoJsonCountryLoader.CountryFeature} whose projected
	 *         polygon contains the mouse position, or {@code null} if no country
	 *         was hit
	 */
