...
/**
 * A simple utility component that wraps a {@link JSlider} and an optional label
 * that displays the current slider value.
 *
 * <p>
 * The {@code RangeSlider} supports two callback hooks:
 * <ul>
 * <li>{@link #setOnChange(Consumer)} – invoked continuously as the user moves
 * the slider</li>
 * <li>{@link #setOnFinalChange(Consumer)} – invoked once after the user stops
 * adjusting</li>
 * </ul>
 *
 * This makes the component suitable for interactive UI (continuous updates) and
 * more expensive model / rendering updates (on final change).
 */
...
public class RangeSlider extends JPanel {
	 ...
	 */
	public RangeSlider(int min, int max, int defaultVal, int majorTick, int minorTick) {
	...
	/**
	 * Returns the underlying JSlider for users who need to add custom label tables
	 * or further tweak the component.
	 */
