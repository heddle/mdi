 ...
 * one of three layout modes:
 * {@link #AS_ORDERED}, {@link #ALPHABETICAL}, or {@link #MINSIZE}.
 * <p>
 * Columns are sized only as wide as necessary to fit the longest label in
 * that column. Horizontal gaps between columns are controlled by
 * {@link #HGAP}, and vertical spacing is based on the font height plus
 * {@link #VGAP}. The "active" checkbox icons are vertically aligned within
 * each column.
 */
...
public class OptionPanel extends JPanel {
    ...
    /**
     * Layout option: minimize overall width by grouping widest labels into the
     * same column. Internally, options are sorted by descending label width,
     * then grouped into columns.
     */
     ...
     * <p>
     * This is a convenience wrapper around {@link ItemListener} that provides
     * the label and new selection state of the changed option.
     */
     ...
     *
     * @param listener     listener notified when any checkbox changes state; may be {@code null}
     * @param columnCount  desired number of columns (must be &gt;= 1)
     * @param font         font used for the checkbox labels (must not be {@code null})
     * @param foreground   foreground color for the labels (may be {@code null} for default)
     * @param background   background color for the panel and checkboxes (may be {@code null} for default)
     * @param layoutOption layout option, one of {@link #AS_ORDERED},
     *                     {@link #ALPHABETICAL}, or {@link #MINSIZE}
     * @param optionData   array of option specifications:
     *                     {@code {label, enabled, selected}} for each option
     *
     * @throws IllegalArgumentException if {@code columnCount < 1}, {@code font} is
     *                                  {@code null}, or {@code optionData} is invalid
     */
    public OptionPanel(OptionPanelListener listener,
                       int columnCount,
                       Font font,
                       Color foreground,
                       Color background,
    ...
    /**
     * Computes the layout (column and row indices, column widths, and preferred size)
     * and sets component bounds accordingly.
     */
    private void layoutEntries() {
        ...
        // Determine how many columns we will actually use (cannot exceed number of options)
