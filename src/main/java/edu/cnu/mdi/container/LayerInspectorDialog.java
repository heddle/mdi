...
/**
 * A lightweight inspector dialog for viewing and editing a container's
 * z-layers.
 * <p>
...
public class LayerInspectorDialog extends JDialog {
	...
	private void deleteUserLayer(Layer layer, boolean moveItemsToDefault) {
		...
		if (moveItemsToDefault) {
			...
			// Move items using normal remove/add. This will fire the standard layer
			// notifications,
			// and will invoke prepareForRemoval() on the source removal path as per
			// Layer.remove().
			// If you later decide you truly need "silent" moves, we can wire those in at
			// the
	 ...
	 * <p>
	 * The table contains protected rows at bottom/top; only user-layer rows may be
	 * moved.
	 */
	private static final class LayerRowTransferHandler extends TransferHandler {
		...
		public boolean canImport(TransferSupport support) {
			...
			if (srcLayer == null || container.isProtectedLayer(srcLayer)
					|| srcLayer == container.getDefaultLayer() && container._layers.size() == 1) {
				return false;
			}
		...
		public boolean importData(TransferSupport support) {
			...
			// Table is bottom->top: [0]=Connections, [1..userCount]=user layers,
			// [last]=Annotations
