...
/**
 * A state-machine driven simulation engine that runs computation on a dedicated thread
 * and delivers UI callbacks on the Swing/AWT Event Dispatch Thread (EDT).
 * <p>
 * This engine is MDI-agnostic. Views can subscribe as {@link SimulationListener}s.
 * </p>
 *
 * <h2>Threading</h2>
 * <ul>
 *   <li>The simulation thread runs {@link Simulation#init(SimulationContext)} and {@link Simulation#step(SimulationContext)}.</li>
 *   <li>All listener callbacks are delivered on the EDT using {@link SwingUtilities#invokeLater(Runnable)}.</li>
 * </ul>
 *
 * <h2>Control</h2>
 * Supports pause/resume, stop, and cancel. Cancellation is cooperative and observed via
 * {@link SimulationContext#isCancelRequested()}.
 */
public final class SimulationEngine {
    ...
    /**
     * Start the engine. Calling start multiple times is safe; only the first call starts a thread.
     */
     ...
     * <p>
     * If paused, this resumes. If currently READY and autoRun is false, this starts running.
     * </p>
     */
    ...
    /**
     * Request pause. The engine will enter {@link SimulationState#PAUSED} at the next safe point.
     */
    ...
    /**
     * Request a normal stop. The engine will terminate and call {@link Simulation#shutdown(SimulationContext)}.
     */
     ...
     *   <li>{@link Simulation#cancel(SimulationContext)} is called on the simulation thread</li>
     * </ul>
     */
     ...
     * <p>
     * This method coalesces rapid calls (if many are posted quickly, the EDT will receive the latest soon).
     * </p>
     *
    ...
    /**
     * Request a transition into SWITCHING state, typically when changing internal phases.
     * <p>
     * This can be used by the simulation owner (e.g., a controller) to annotate major phases.
     * </p>
     *
        ...
        try {
            ...
            while (!stopRequested) {
                ...
                if (config.refreshIntervalMs > 0 &&
                        (now - lastRefresh) >= config.refreshIntervalMs * 1_000_000L) {
                    lastRefresh = now;
                    requestRefresh();
                }
                ...
                if (config.progressIntervalMs > 0 &&
                        (now - lastProgress) >= config.progressIntervalMs * 1_000_000L) {
                    lastProgress = now;
            ...
            // Termination
            transition(SimulationState.TERMINATING, context.isCancelRequested() ? "cancel" : (stopRequested ? "stop" : "complete"));
            try {
