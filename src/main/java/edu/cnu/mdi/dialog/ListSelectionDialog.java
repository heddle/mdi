...
public class ListSelectionDialog<T> extends JDialog implements ActionListener {
	 ...
	 * @param selectionMode the mode should be one of the relevant constants from
	 *                      the ListSelectionModel class
	 * @param preferredSize the preferred size selection list. If null, 300x400
	 *                      used.
	 * @param items         the list of items
	 */
	public ListSelectionDialog(JFrame parent, String title, boolean modal, int selectionMode, Dimension preferredSize,
			List<T> items) {
		...
		WindowAdapter wa = new WindowAdapter() {
			...
			public void windowClosing(WindowEvent we) {
				reason = DialogUtils.CANCEL_RESPONSE;
				setVisible(false);
			}
		};
	...
	public void actionPerformed(ActionEvent e) {
		reason = e.getActionCommand().equals(CANCELSTR) ? DialogUtils.CANCEL_RESPONSE : DialogUtils.OK_RESPONSE;
		setVisible(false);
	}
	...
	public static void main(String arg[]) {
		...
		String names[] = { "Item 1", "Item 2", "Item 3", "Item 4", "Item 5", "Item 6", "Item 7", "Item 8", "Item 9",
...
//    			int selectionMode, Dimension preferredSize, List<T> items) {
		...
		ListSelectionDialog<TestObject> dialog = new ListSelectionDialog<>(null, "Sample Selection", true,
				ListSelectionModel.SINGLE_SELECTION, new Dimension(300, 400), objects);
		...
		try {
			EventQueue.invokeAndWait(new Runnable() {
				...
				public void run() {
					...
					if (dialog.getReason() == DialogUtils.OK_RESPONSE) {
