...
public class Triangle3D extends Item3D {
	 ...
	 * @param frame     frame the triangle
	 */
	public Triangle3D(Panel3D panel3d, float x1, float y1, float z1, float x2, float y2, float z2, float x3, float y3,
			float z3, Color color, float lineWidth, boolean frame) {
		super(panel3d);
		_coords = new float[9];
		_coords[0] = x1;
		_coords[1] = y1;
