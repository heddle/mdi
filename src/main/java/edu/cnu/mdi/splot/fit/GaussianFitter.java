...
public final class GaussianFitter extends ALeastSquaresFitter {
	...
	/**
	 * Default minimum allowed sigma to avoid division by zero and ill-conditioned
	 * Jacobians.
	 */
	...
	protected ParameterValidator defaultValidator() {
		...
		double[] lo = { Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY, DEFAULT_MIN_SIGMA,
	 ...
	 *
	 * <p>
	 * If {@code bounds} is null, only {@code sigma >= DEFAULT_MIN_SIGMA} is
	 * enforced.
	 * </p>
	 *
	 * @param x            x data
	...
	public FitResult fit(double[] x, double[] y, double[] weights, ParameterBounds bounds, double[] initialGuess) {
		...
		if (bounds == null) {
			lo = new double[] { Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY, DEFAULT_MIN_SIGMA,
					Double.NEGATIVE_INFINITY };
			hi = new double[] { Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY,
					Double.POSITIVE_INFINITY };
		} else {
	...
	/**
	 * Simple bounds container for the Gaussian parameters. Use +/-infinity for
	 * unbounded.
	 */
	...
	public String functionForm() {
		return String.format("y(x)=%se^[-(x-%s)%s/(2%s%s)] + %s", paramNames[0], paramNames[1], SUP2, paramNames[2],
				SUP2, paramNames[3]);
	}
