...
public final class PolynomialFitter extends ALeastSquaresFitter implements IFitter {
	...
	/**
	 * Create a polynomial fitter for a given degree using Levenberg-Marquardt and a
	 * robust default guess.
	 *
	...
	public PolynomialFitter(int degree, LeastSquaresOptimizer optimizer) {
		// For polynomials, we prefer the weighted linear LS guess over generic
		// heuristics.
		// Still pass a non-null initialGuesser to satisfy base class invariants; it is
		// used only
		// if defaultInitialGuess(...) is overridden in the future or returns null (it
		// does not).
	 ...
	 *
	 * <p>
	 * This is an "expert" overload that adapts polynomial coefficient bounds to the
	 * base class {@link ParameterValidator} mechanism.
	 *
	 * @param x            x data
	 * @param y            y data
	 * @param weights      optional weights (length n). Typically weights =
	 *                     1/sigmaY^2.
	 * @param bounds       optional coefficient bounds; if null, unbounded
	 * @param initialGuess optional coefficients length degree+1; if null, uses
	 *                     default guess
	 * @return FitResult
	 */
	...
	/**
	 * Create an {@link Evaluator} that evaluates this polynomial using Horner's
	 * method.
	 *
	...
	/**
	 * Build a weighted (or unweighted) linear least-squares initial guess for
	 ...
	 *
	 * <p>
	 * This is retained as a polynomial-friendly API, but the actual enforcement is
	 * handled through the base class {@link ParameterValidator} mechanism (via
	 * clamping).
	 */
