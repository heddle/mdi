...
/**
 * Utility for computing a weighted linear least-squares parameter guess: min
 * sum_i w_i (A_i p - y_i)^2 by solving (sqrt(W)A)p = sqrt(W)y via QR.
 */
public final class LinearLeastSquaresGuesser {
	...
	public static double[] solve(double[][] A, double[] y, double[] weights) {
		...
		try {
			DecompositionSolver solver = new QRDecomposition(new Array2DRowRealMatrix(Aw, false)).getSolver();
