 ...
 *
 * <p>
 * This class centralizes the common fit pipeline:
 * <ul>
 ...
 *
 * <p>
 * Concrete fitters typically only implement:
 * <ul>
...
public abstract class ALeastSquaresFitter implements IFitter, IFitStringGetter {
	...
	/**
	 * Initial-guess strategy used when the caller does not provide an explicit
	 * guess.
	 */
	...
	/**
	 * Covariance extraction threshold passed to
	 * {@code Optimum.getCovariances(threshold)}. Subclasses may override via
	 * {@link #getCovarianceThreshold()}.
	 */
	...
	protected ALeastSquaresFitter(LeastSquaresOptimizer optimizer, IInitialGuess initialGuesser) {
		this(optimizer, initialGuesser, 2000, 2000, new SimpleVectorValueChecker(1e-12, 1e-12), 1e-14);
	}
	 ...
	 */
	protected ALeastSquaresFitter(LeastSquaresOptimizer optimizer, IInitialGuess initialGuesser, int maxIterations,
			int maxEvaluations, SimpleVectorValueChecker checker, double covarianceThreshold) {
	...
	/**
	 * Build the model + analytic (or numeric) Jacobian for this fitter, given x
	 * values.
	 *
	 * @param x x data (length n)
	 * @return a {@link MultivariateJacobianFunction} compatible with Commons Math
	 *         least-squares
	 */
	protected abstract MultivariateJacobianFunction model(double[] x);

	/**
	 * Default initial guess used if the caller does not provide an explicit
	 * override. The default implementation delegates to the
	 * {@link #initialGuesser}.
	 *
	 * <p>
	 * Subclasses may override if they have a better model-specific guess.
	 *
	 * @param x       x data
	 * @param y       y data
	 ...
	 *
	 * <p>
	 * Return null for "no validation". For simple bound constraints, consider using
	 ...
	 *
	 * <p>
	 * Commons Math accepts any real diagonal matrix as weights, but for typical
	 * least-squares usage, weights should be finite and non-negative (often
	 * {@code 1/sigma^2}).
	 *
	...
	/**
	 * Create a {@link ParameterValidator} that clamps parameters to
	 * {@code [lower[i], upper[i]]}. Use +/-infinity for "no bound".
	 *
	 * <p>
	 * This is a simple, robust bounds strategy compatible with Commons Math
	 * least-squares. It is not a hard-constraint optimizer; it repairs invalid
	 * steps by clamping.
	 *
	...
	/**
	 * Best-effort covariance extraction; returns null if unavailable (singular,
	 * ill-conditioned, etc.).
	 */
	...
	/**
	 * Fit with no weights, no initial guess override, and no parameter validator
	 * override.
	 *
	 * @param x the x data
	 * @param y the y data
	...
	/**
	 * Fit with weights but no initial guess override and no parameter validator
	 * override.
	 *
	 * @param x       the x data
	 * @param y       the y data
	...
	/**
	 * Full fit entry point. This is the common least-squares pipeline for all
	 * derived fitters.
	 *
	 * @param x                    x data
	 * @param y                    y data
	 * @param weights              optional weights (length n), typically
	 *                             {@code 1/sigmaY^2}; may be null
	 * @param initialGuessOverride optional explicit initial guess (length p); may
	 *                             be null
	 * @param validatorOverride    optional explicit parameter validator; may be
	 *                             null
	 * @return FitResult
	 */
	public final FitResult fit(double[] x, double[] y, double[] weights, double[] initialGuessOverride,
		...
		final LeastSquaresBuilder b = new LeastSquaresBuilder().start(start).model(m).target(y)
				.maxIterations(getMaxIterations()).maxEvaluations(getMaxEvaluations());
