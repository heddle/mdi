 ...
 * <li>Build an approximate derivative spline via numeric differentiation
 * {@link #derivative()}.</li>
 * <li>Find roots in a range using bracket-and-refine utilities
 * {@link #findRoots(double, double)}.</li>
 * <li>Find maxima in a range by locating zeros of the first derivative and
 * testing the second derivative
 * {@link #findXValsOfMaxima(double, double)}.</li>
 * </ul>
 *
 * <h2>Spline details</h2> The spline is "natural": the second derivative is
 * forced to be zero at the endpoints. Between consecutive knots
 * {@code x[i] <= x <= x[i+1]}, the interpolant is a cubic polynomial chosen so
 * that {@code y}, {@code y'} and {@code y''} are continuous across knots.
 *
 * <h2>Requirements</h2>
 * <ul>
 * <li>{@code x[]} and {@code y[]} must be non-null, same length, and length
 * {@code >= 2}.</li>
 * <li>{@code x[]} must be strictly increasing (no duplicates).</li>
 * </ul>
 *
 * <h2>Out-of-range evaluation</h2> If {@code x} is outside
 * {@code [x[0], x[n-1]]}, this implementation clamps to the nearest endpoint
...
public class CubicSpline implements Evaluator {
	...
	/**
	 * Precomputed second derivatives at knots for the natural cubic spline. This is
	 * the classic {@code y2[]} array from Numerical Recipes' spline implementation.
	 */
	 ...
	 * @throws IllegalArgumentException if inputs are invalid (null, length
	 *                                  mismatch, length &lt; 2, non-increasing x)
	 */
	 ...
	 * <p>
	 * The interval containing {@code x} is found with a binary search, then the
	 * standard cubic-spline interpolation formula is used.
	 *
	 * @param x query x-value
	 * @return interpolated y-value; clamped to endpoints if {@code x} is outside
	 *         the knot range
	 */
	@Override
	public double value(double x) {
		...
		// Binary search for the right interval: find klo such that x[klo] <= x <
		// x[klo+1]
		int klo = 0;
		int khi = n - 1;
		while (khi - klo > 1) {
	...
	/**
	 * Obtain a cubic spline that approximates the derivative of this spline by
	 * numeric differentiation at the knots, then spline-interpolating those
	 * derivative samples.
	 * <p>
	 ...
	 * </ul>
	 * This is robust and simple. If you need an analytic derivative of the spline
	 * itself, you could compute it directly from the spline coefficients; this
	 * method instead builds a new spline based on sampled slopes at knots.
	 *
	...
	/**
	 * Find the {@code (x, y)} values of the maxima of the interpolated function in
	 * a given range.
	 * <p>
	 ...
	 * <li>Keep only critical points where the second derivative is negative (local
	 * maxima).</li>
	 * <li>Return the corresponding {@code (x, f(x))} pairs, sorted by descending
	 * {@code y} if there is more than one.</li>
	 * </ol>
	 *
	 ...
	 * <p>
	 * This is the standard tridiagonal solve for {@code y2[]} with boundary
	 * conditions {@code y2[0]=y2[n-1]=0}.
	 */
