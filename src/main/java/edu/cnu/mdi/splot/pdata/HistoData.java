 ...
 * <p>
 * Bin edges are stored in {@code grid[]} of length (numBins + 1). Counts are
 * stored in {@code counts[]} of length numBins.
 *
 * <h3>Fit preparation helpers</h3> This class includes helpers to build x/y
 * vectors for least-squares fitting:
 * <ul>
 * <li>x[i] = bin center</li>
 * <li>y[i] = bin count</li>
 * <li>optional Poisson weights: w[i] = 1/count for count&gt;0</li>
 * </ul>
 * It also includes several peak-finding strategies and "prepare around peak"
 * convenience methods, including guarded versions that deal with edge proximity
 * and minimum-point constraints.
 *
 * @author heddle
 */
public class HistoData {
	 ...
	 * rms in index 2. Null means cache is invalid and must be recomputed.
	 */
	...
	/**
	 * Get the number of entries in the histogram (excluding underflows and
	 * overflows).
	 *
	 ...
	 * statistics once. Counts outside range are accumulated into under/over-flow
	 * counts.
	 * </p>
	 *
	...
	/**
	 * Set a bin to a given count (value determines which bin). Counts outside range
	 * are accumulated into under/over counts.
	 */
	...
	/**
	 * Fit prep result that also reports which peak bin and window were used. Useful
	 * for status/debug strings in the UI.
	 */
	public static final class FitWindowData extends FitVectors {
		...
		public FitWindowData(double[] x, double[] y, double[] weights, int peakBin, int bin0, int bin1,
	 ...
	 * <li>{@code weights[i]} (optional) = 1/sigmaY^2 using Poisson sigmaY =
	 * sqrt(count)</li>
	 * </ul>
	 */
	...
	/**
	 * Prepare arrays suitable for fitters from a symmetric window around a peak
	 * bin.
	 */
	...
	/**
	 * Find the (0-based) bin index with the maximum count (raw). If there are
	 * multiple maxima, returns the first.
	 *
	...
	/**
	 * Find the (0-based) bin index with the maximum count within an inclusive bin
	 * range. If there are multiple maxima, returns the first.
	 */
	 ...
	 * <li>If multiple adjacent bins tie (plateau), pick the most sensible raw bin
	 * within plateau</li>
	 * </ul>
	 */
		...
		for (int i = b0; i <= b1; i++) {
			...
			if (score > bestScore) {
				...
				if (i == p1 + 1) {
					...
					// Non-contiguous tie: keep the plateau segment whose chosen raw representative
					// is better.
	...
	public FitVectors prepareForFitAroundSmoothedPeak(boolean includeZeroBins, int halfWindowBins, int smoothRadius,
			boolean ignoreZeroBins, boolean poissonWeights) {
	...
	public FitVectors prepareForFitAroundSmoothedPeak(boolean includeZeroBins, int bin0, int bin1, int halfWindowBins,
			int smoothRadius, boolean ignoreZeroBins, boolean poissonWeights) {
	 ...
	 * <ul>
	 * <li>edge sanity: prefer peaks that can support the requested window
	 * fully</li>
	 * <li>auto-shrink if peak is near edges</li>
	 * <li>min-points safety after filtering; expand window to max if needed;
	 * optionally include zeros as last resort</li>
	 * </ul>
	 *
	 * @param includeZeroBins include bins with count==0 in returned fit vectors (if
	 *                        false, zeros are skipped)
	 * @param searchBin0      inclusive start bin to search for peak (0-based;
	 *                        clamped)
	 * @param searchBin1      inclusive end bin to search for peak (0-based;
	 *                        clamped)
	 * @param halfWindowBins  requested half-window size (>= 0)
	 * @param smoothRadius    smoothing radius for
	 *                        {@link #findPeakBinBest(int, int, int, boolean)} (>=
	 *                        0)
	 * @param ignoreZeroBins  if true, zero-count bins are skipped as PEAK
	 *                        candidates
	 * @param poissonWeights  if true, include Poisson weights
	 * @param minPoints       minimum number of points required after filtering (>=
	 *                        1)
	 */
	public FitWindowData prepareForFitAroundBestPeakGuarded(boolean includeZeroBins, int searchBin0, int searchBin1,
			int halfWindowBins, int smoothRadius, boolean ignoreZeroBins, boolean poissonWeights, int minPoints) {
		int nbin = getNumberBins();
		if (nbin <= 0) {
			return new FitWindowData(new double[0], new double[0], poissonWeights ? new double[0] : null, -1, 0, -1, 0);
		}
		if (halfWindowBins < 0) {
			throw new IllegalArgumentException("halfWindowBins must be >= 0");
		}
		...
		return new FitWindowData(new double[0], new double[0], poissonWeights ? new double[0] : null, peak, b0, b1,
				halfUsed);
	}

	/** Convenience: guarded best-peak search over whole histogram. */
	public FitWindowData prepareForFitAroundBestPeakGuarded(boolean includeZeroBins, int halfWindowBins,
			int smoothRadius, boolean ignoreZeroBins, boolean poissonWeights, int minPoints) {
		return prepareForFitAroundBestPeakGuarded(includeZeroBins, 0, getNumberBins() - 1, halfWindowBins, smoothRadius,
	...
	private FitWindowData buildWindow(boolean includeZeroBins, int peak, int halfUsed, int s0, int s1,
			boolean poissonWeights) {
		int b0 = Math.max(s0, peak - halfUsed);
		int b1 = Math.min(s1, peak + halfUsed);
	...
	public String toString() {
		...
		sb.append("{bins=").append(nbin).append(", x=[").append(xmin).append(", ").append(xmax).append("]")
				.append(", good=").append(good).append(", under=").append(underCount).append(", over=")
				.append(overCount).append(", total=").append(total);
		...
		if (peak >= 0) {
			sb.append(", peakBin=").append(peak).append(", peakX=").append(peakX).append(", peakCount=")
					.append(peakCount);
		}
