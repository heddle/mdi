 ...
 *
 * <p>
 * This example is intentionally designed as a <em>thread stress test</em> that
 * simulates a streaming data-acquisition (DAQ) system.
 * </p>
 *
 * <h2>What is being tested</h2>
 * <ul>
 * <li>Many background producer threads concurrently call
 * {@link Curve#enqueue(double, double)}.</li>
 * <li>The EDT periodically drains queued points into the curve's data columns
 * using {@link Curve#startPendingDrainTimer(int, int)} (which calls
 * {@link Curve#drainPendingOnEDT(int)}).</li>
 * <li>Rendering/fitting happens as usual on the EDT with a consistently-mutated
 * model.</li>
 * </ul>
 *
 * <p>
 * This is the correct way to stress concurrency <b>without</b> reintroducing
 * races between painting and model mutation. The old “unsafe mode” (background
 * threads calling {@code curve.add(...)}) is now prevented by EDT guards and
 * should fail fast.
 * </p>
 *
 * <h2>Key design choice</h2>
 * <p>
 * {@link #fillData()} is intentionally a no-op. Data arrives over time via
 * producers.
 * </p>
 *
 * <h2>Tuning knobs</h2>
 ...
 * <li>{@link #MAX_POINTS} - max points retained in the curve (demonstrates stop
 * enqueuing)</li>
 * </ul>
 */
...
public class Scatter extends AExample {
	...
	/**
	 * Per-producer delay between points (ms). Smaller means higher rate and more
	 * stress.
	 */
	 ...
	 * No-op by design.
	 * <p>
	 * This example simulates streaming acquisition rather than batch
	 * initialization. Data arrives asynchronously via background producer threads.
	 * </p>
	 */
	@Override
	...
	private void startDaq() {
		...
		SwingUtilities.invokeLater(() -> c.startPendingDrainTimer(DRAIN_PERIOD_MS, MAX_DRAIN_PER_TICK, drained -> {
	 ...
	 * <p>
	 * Generates random points and adds them. This intentionally creates contention
	 * on the lock-free queue and stresses the thread safety.
	 * </p>
	 *
