 ...
 *   <li><b>Locked</b> layers are still drawn (if visible), but their items are treated
 *       as non-interactive: hit-testing returns {@code null}, selection helpers ignore
 *       them, and deletion helpers no-op.</li>
 * </ul>
 *
 * <p>
 * <b>Threading:</b> mutating operations synchronize on {@code this} to provide basic
 * safety when tools are mutating items while the UI thread is drawing.
 *
 * @author heddle
 */
...
public class Layer extends ArrayList<AItem> {
    ...
    /**
     * Layer-level lock. When {@code true}, the layer's items are treated as non-interactive.
     * <p>
     * Note: item-level locks still apply independently (see {@link AItem#isLocked()}).
     */
     ...
     * <p>
     * If {@code o} is an {@link AItem}, {@link AItem#prepareForRemoval()} is called first.
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * Intentionally a no-op. Use {@link #clearAllItems(IContainer)} if you want
     * a safe "clear" that detaches feedback providers and honors deletable flags.
     */
    @Override
    public void clear() {
        // no-op by design
    ...
    /**
     * Clear all deletable items from this layer, properly detaching feedback providers.
     * <p>
     * This is safer than {@link #clear()} because items may also be registered with
     * container-managed services (e.g., feedback control).
     *
     * @param container the owning container (may be null; falls back to this layer's container)
     */
    ...
    /**
     * Remove an item without firing change notifications or calling prepareForRemoval().
     * <p>
     * Intended for container-level operations like moving items between layers.
     */
