 ...
 * interaction layer, and can be dragged, resized, rotated, selected, or deleted
 * according to a set of independently-controllable behavior flags.
 * </p>
 *
 ...
 * </p>
 * <ul>
 * <li>{@link #drawItem(Graphics2D, IContainer)} — performs the actual rendering
 * of the item's geometry.</li>
 * <li>{@link #shouldDraw(Graphics2D, IContainer)} — returns {@code true} if the
 * item should be drawn at all (e.g. it intersects the visible area).</li>
 * </ul>
 *
 * <h2>Behavior flags and the lock</h2>
 * <p>
 * Each interactive behavior (dragging, resizing, rotating, deleting) is
 * individually controlled by a boolean flag. However, the {@link #_locked} flag
 * is a master override: when {@code true} it suppresses dragging, resizing,
 * rotating, and deleting <em>regardless</em> of the individual flags. Locking
 * does not suppress visibility or selection display.
 * </p>
 * <p>
 * <strong>Items are locked by default ({@code _locked = true}).</strong> Call
 * {@link #setLocked(boolean) setLocked(false)} or supply
 ...
 * attributes, and display name. Subclasses that need geometry parameters should
 * override {@link #applyGeometryProperties(Properties)}.
 * </p>
 *
 * <h2>Geometry</h2>
 * <p>
...
public abstract class AItem implements IDrawable, IFeedbackProvider {
	...
	protected static Icon rotateIcon = ImageManager.getInstance()
			.loadUiIcon(Environment.MDI_RESOURCE_PATH + "images/svg/rotate.svg", 16);

	// fill and line colors for selection and rotation handles
	...
	// flag controlling whether the focus indicator is drawn; default true
	private boolean _fillFocus = true;
	...
	/**
	 * World-coordinate focus — typically the centroid or anchor. Drawn as a small
	...
	/**
	 * The drawing style (colors, line style, symbol type, etc.). Never {@code null}
	 * under normal operation; may be {@code null} after
	 * {@link #prepareForRemoval()}.
	 *
	 * @see #getStyleSafe()
	 */
	...
	/**
	 * Master lock flag. When {@code true} drag, resize, rotate, and delete are all
	 * suppressed, regardless of the individual flags.
	 * <p>
	 ...
	 * <p>
	 * This field is {@code protected} (not {@code private}) so that subclasses can
	 * read the raw flag value independently of the lock — for example, to include
	 * it in a serialised representation. The effective resizability is always
	 * exposed through {@link #isResizable()}, which also checks {@link #_locked}.
	 * </p>
	 */
	...
	/**
	 * Whether the item's cached rendering data is stale. Setting this to
	 * {@code true} also clears {@link #_lastDrawnPolygon}.
	 */
	...
	/**
	 * The screen polygon from the most recent {@link #drawItem} call. {@code null}
	 * when dirty or not yet drawn. Used for hit-testing and rubber-band selection.
	 */
	...
	/**
	 * Rotation angle in degrees, range {@code (-180, 180]}. Normalised by
	 * {@link #setAzimuth(double)}.
	 */
	...
	/**
	 * Construct an item on the given layer, optionally configuring it from property
	 * key-value pairs, and immediately register it.
	 *
	 * <p>
	 * Registration adds the item to its layer (firing an
	 * {@link ItemChangeType#ADDED} event) and registers it as a feedback provider
	 * with the container's feedback control. Registration happens inside the
	 * constructor so that callers — including factory methods such as those in
	 * {@code CreationSupport} — do not need to take any extra step after
	 * construction.
	 * </p>
	 *
	 * <p>
	 * Construction-time event caveat
	 * </p>
	 * <p>
	 ...
	 * {@link #applyProperties} has set — not any geometry a subclass sets after
	 * {@code super()} returns. In practice, MDI listeners that care about geometry
	 * wait for a subsequent draw or modification event, so this has not been a
	 * problem. If a future listener does need to read geometry on {@code ADDED},
	 * the creation call-site should set geometry before adding the listener, or the
	 * listener should guard against uninitialized state.
	 * </p>
	 *
	 * @param layer   the layer to add this item to; must not be {@code null}
	 * @param keyVals optional alternating {@link PropertyUtils} key/value pairs
	...
	/**
	 * Apply a set of properties to this item's behavior flags, style, name, and
	 * geometry.
	 *
	 * <p>
	 * Called from the Phase-1 constructor. May also be called later to reconfigure
	 * the item. Order: flags → name → style → geometry.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * Each property is applied only when the key is present; missing keys leave the
	 * current style unchanged. Subclasses that support additional style properties
	 * should override this method and call
	 * {@code super.applyStyleProperties(props)} first.
	 * </p>
	 *
	...
	protected void applyStyleProperties(Properties props) {
		...
		if (props.containsKey(PropertyUtils.FILLCOLOR))
			s.setFillColor(PropertyUtils.getFillColor(props));
		if (props.containsKey(PropertyUtils.LINECOLOR))
			s.setLineColor(PropertyUtils.getLineColor(props));
		if (props.containsKey(PropertyUtils.LINESTYLE))
			s.setLineStyle(PropertyUtils.getLineStyle(props));
		if (props.containsKey(PropertyUtils.LINEWIDTH))
			s.setLineWidth(PropertyUtils.getLineWidth(props));
		if (props.containsKey(PropertyUtils.SYMBOL))
			s.setSymbolType(PropertyUtils.getSymbol(props));
		if (props.containsKey(PropertyUtils.SYMBOLSIZE))
			s.setSymbolSize(PropertyUtils.getSymbolSize(props));
		if (props.containsKey(PropertyUtils.TEXTCOLOR))
			s.setTextColor(PropertyUtils.getTextColor(props));
	}
	 ...
	 *
	 * <p>
	 * Default implementation is a no-op. Subclasses with geometric constructor
	 * parameters (radius, width, endpoint coordinates, etc.) should override this
	 * rather than adding constructor parameters.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * This is the method called by the container's rendering pipeline. Subclasses
	 * must <em>not</em> override it; implement
	 * {@link #drawItem(Graphics2D, IContainer)} instead.
	 * </p>
	 *
	 * <p>
	 * Steps performed:
	 ...
	 * <li>Call {@link #shouldDraw} — skip {@link #drawItem} if {@code false}.</li>
	 * <li>Install any special clip from the owning {@link BaseView}.</li>
	 * <li>Save and restore the {@link Stroke} around {@link #drawItem}.</li>
	 * <li>Call {@link #drawSelections} unconditionally (selected items need handles
	 * even when scrolled partially out of view).</li>
	 * </ol>
	 *
	 ...
	 *
	 * <p>
	 * Called by {@link #draw} after visibility and clip setup. The stroke is saved
	 * and restored by {@code draw}, so subclasses may freely change the stroke
	 * without cleanup.
	 * </p>
	 *
	 * @param g2        the graphics context
	 ...
	 *
	 * <p>
	 * This is a secondary visibility check beyond the simple {@link #_visible}
	 * flag. A typical implementation checks whether the item's screen bounds
	 * intersect the current clip rectangle.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * Called unconditionally by {@link #draw} whenever the item is visible, even
	 * when {@link #shouldDraw} returned {@code false}, so handles remain visible
	 * for selected items that are partially scrolled out of view.
	 * </p>
	 *
	 * <p>
	 * When selected, draws:
	...
	public void drawSelections(Graphics2D g2, IContainer container) {
		...
		if (isRotatable()) {
			...
			if (rp != null && rotateIcon != null) {
				...
				rotateIcon.paintIcon(container.getComponent(), g2, rp.x - rotateIcon.getIconHeight() / 2,
	...
	/**
	 * Return whether the focus indicator is drawn when selected.
	 *
	 * @return {@code true} if the focus indicator is drawn
	 */
	public void setFillFocus(boolean fillFocus) {
		_fillFocus = fillFocus;
	}
	 ...
	 *
	 * <p>
	 * Only drawn when {@link #getFocus()} returns a non-null point. The indicator
	 * is a 6×6 pixel square filled with {@link #_FOCUSFILL}.
	 * </p>
	 *
	 * @param g2        the graphics context
	 * @param container the container being rendered
	...
	/**
	 * Test whether the screen point falls inside any selection or rotation handle
	 * rectangle.
	 *
	 * @param container   the container rendering this item
	 * @param screenPoint the pixel point to test
	...
	protected boolean inASelectRect(IContainer container, Point screenPoint) {
		return inSelectPoint(container, screenPoint, false) >= 0 || inRotatePoint(container, screenPoint);
	}
	 ...
	 *
	 * <p>
	 * Returns {@code false} immediately if the item is not rotatable or not
	 * selected.
	 * </p>
	 *
	 * @param container   the container rendering this item
	...
	public boolean inRotatePoint(IContainer container, Point screenPoint) {
		...
		if (p == null)
			return false;
		return new Rectangle(p.x - RPSIZE2, p.y - RPSIZE2, RPSIZE, RPSIZE).contains(screenPoint);
	}

	/**
	 * Return the index of the selection handle whose hit-test rectangle contains
	 * the given screen point, or {@code -1} if none does.
	 *
	 * <p>
	 * Returns {@code -1} immediately if the item is not selected, or if
	 ...
	 *
	 * <p>
	 * The default implementation uses the cached screen polygon if available,
	 * otherwise the bounding rectangle. Subclasses with non-rectangular shapes may
	 * override for a more precise test.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * The default implementation returns {@code null}. Subclasses should override
	 * to return a meaningful bounding box used for hit-testing and rubber-band
	 * selection.
	 * </p>
	 *
	 * @param container the container rendering this item
	 ...
	 *
	 * <p>
	 * The default uses the vertices of {@link #_lastDrawnPolygon} if available,
	 * otherwise the four corners of {@link #getBounds}.
	 * </p>
	 *
	 * @param container the container rendering this item
	...
	public Point[] getSelectionPoints(IContainer container) {
		...
		return new Point[] { new Point(r.x, r.y), new Point(r.x, bottom), new Point(right, bottom),
				new Point(right, r.y) };
	}

	/**
	 * Return the screen-coordinate rotation handle point, or {@code null} if the
	 * item does not support rotation.
	 *
	 * @param container the container rendering this item
	 ...
	 *
	 * <p>
	 * The focus is the item's conceptual center: for point items it is their
	 * location; for polygon items it is typically the centroid. Returns
	 * {@code null} if no focus has been set.
	 * </p>
	 *
	...
	/**
	 * Return the screen-coordinate location of the focus, or {@code null} if the
	 * focus is not set.
	 *
	...
	/**
	 * Hook called when geometry changes and the focus should be recomputed. The
	...
	/**
	 * Notify the item that its geometry has changed. Recomputes the focus and marks
	 * the item dirty.
	 */
	 ...
	 *
	 * <p>
	 * Pixel deltas are converted to world deltas via the container transform, then
	 * delegated to {@link #translateWorld}.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * Priority: rotate (if start point is in rotation handle) → resize (if start
	 * point is in a selection handle) → drag (otherwise).
	 * </p>
	 *
	 * <p>
	...
	/**
	 * Update this item's geometry in response to a mouse movement during an active
	 * modification.
	 *
	 * <p>
	 * Called repeatedly while the user is dragging, resizing, or rotating. The
	 * current mouse position is available through {@link #_modification}.
	 * </p>
	 */
	 ...
	 *
	 * <p>
	 * Fires {@link ItemChangeType#MOVED}, {@link ItemChangeType#ROTATED}, or
	 * {@link ItemChangeType#RESIZED} depending on the modification type. This
	 * method is a no-op if {@link #_modification} is {@code null}.
	 * </p>
	 */
	...
	/**
	 * Return {@code true} if this item can be dragged. Returns {@code false} when
	 * locked, regardless of the drag flag.
	 *
	...
	/**
	 * Return {@code true} if this item can be deleted. Returns {@code false} when
	 * locked.
	 *
	...
	/**
	 * Return {@code true} if this item can be resized. Returns {@code false} when
	 * locked.
	 *
	...
	/**
	 * Return {@code true} if this item can be rotated. Returns {@code false} when
	 * locked.
	 *
	...
	public boolean isStyleEditable() {
		return _styleEditable;
	}

	/** @param styleEditable {@code true} to allow styling via the style editor */
	public void setStyleEditable(boolean styleEditable) {
		_styleEditable = styleEditable;
	}
	 ...
	 *
	 * <p>
	 * A locked item ignores drag, resize, rotate, and delete operations regardless
	 * of its individual flags. Items are locked by default.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * An item is trackable only when it is unlocked, enabled, and at least one of
	 * {@link #_draggable}, {@link #_rotatable}, or {@link #_resizable} is
	 * {@code true}. The interaction layer uses this as a fast pre-check before
	 * performing the more expensive hit-test against individual selection and
	 * rotation handles.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * Passing {@code null} is accepted but will cause {@link NullPointerException}s
	 * in the drawing pipeline unless {@link #getStyleSafe()} is used for all
	 * subsequent style accesses.
	 * </p>
	 *
	...
	/**
	 * Return this item's style, recreating it as a default {@link Styled} instance
	 * if {@link #_style} is {@code null}.
	 *
	 * <p>
	 * Used internally (e.g. in {@link #applyStyleProperties}) to guard against the
	 * style having been cleared by {@link #prepareForRemoval()}. Prefer
	 * {@link #getStyle()} in normal usage.
	 * </p>
	 *
	...
	/**
	 * Return the {@link BaseView} that owns the container hosting this item, or
	 ...
	 *
	 * <p>
	 * The default menu contains an item-ordering sub-menu and a "Locked" checkbox.
	 * Subclasses may override and call {@code super.createPopupMenu()} before
	...
	/**
	 * Called when this item receives a double-click. The default implementation is
	 * a no-op.
	 *
	...
	/**
	 * Populate the feedback string list with information about this item. The
	 * default implementation is a no-op.
	 *
	 * @param container       the container rendering this item
	...
	/**
	 * Identity-based equality: two {@code AItem} references are equal only when
	 ...
	 * </p>
	 * <ol>
	 * <li><b>Feedback de-registration.</b> The item unregisters itself from the
	 * container's {@link edu.cnu.mdi.feedback.FeedbackControl} so that it is no
	 * longer polled after removal. The container reference is captured before
	 * {@link #_layer} is nulled.</li>
	 * <li><b>Reference nulling.</b> Heavyweight references are set to {@code null}
	 * to assist garbage collection and to cause fast, obvious
	 * {@link NullPointerException}s if the item is used after removal.</li>
	 * </ol>
	 *
	 * <p>
	 * Subclasses holding additional heavyweight references should override this
	 * method, null those fields, and call {@code super.prepareForRemoval()} either
	 * first or last.
	 * </p>
	 */
	@Override
