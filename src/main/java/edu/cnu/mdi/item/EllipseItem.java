...
/**
 * An ellipse item represented internally as a {@link java.awt.geom.Path2D} polygonal
 * approximation.
 * <p>
 * Key invariants:
 * <ul>
 ...
 *   <li>Selection handles are the four cardinal points of the ellipse in its rotated
 *       frame (top/right/bottom/left).</li>
 * </ul>
 * </p>
 * <p>
 * Resize correctness after rotation depends on using the same azimuth convention
 * everywhere. In this codebase, azimuth behaves like a compass bearing:
 * <b>0° = north (+y), 90° = east (+x)</b>, increasing clockwise. That convention is
 * used by {@link WorldGraphicsUtils#project(Point2D.Double, double, double, Point2D.Double)}.
 * </p>
 */
public class EllipseItem extends PolygonItem {
     ...
     * @param azimuth rotation in degrees, using the application's compass/bearing convention.
     * @param center  ellipse center in world coordinates (copied). If {@code null}, the
     *                ellipse path is null until focus is set and {@link #rebuildPath()} is called.
     */
    ...
    /**
     * Rebuild the polygonal path approximation from the current size/azimuth and focus (center).
     * <p>
     * Some superclasses compute focus from path centroid; for ellipses we define focus == center,
     * so we restore focus after setting the path.
     * </p>
     */
    ...
    /**
     * Ellipse-specific reshape: update width/height based on which selection handle is dragged.
     * <p>
     * Handles are:
     * <ul>
     *   <li>0 = top</li>
     ...
     * <ul>
     *   <li>Use the <b>start</b> focus as the center for the entire drag to avoid drift.</li>
     *   <li>Project into ellipse-local coordinates using the same <b>bearing azimuth</b>
     *       convention as {@link WorldGraphicsUtils#project}.</li>
     * </ul>
     * </p>
     */
    @Override
    protected void reshape() {
         ...
         *
         * Bearing azimuth β:
         *   unit vector at bearing β is (sinβ, cosβ) in (x,y).
         *
         * In our handle placement:
         *   localY axis (semi-height b) points along bearing azimuth (top/bottom),
         *   localX axis (semi-width a) points along bearing azimuth + 90° (right/left).
         */
    ...
    /**
     * Programmatically set ellipse size (full extents), clamped to {@link #MIN_SIZE}.
     *
