 ...
 * </ul>
 * Endpoints are recomputed from the items each time geometry is updated, so the
 * connection follows its endpoints when nodes move.
 */
public class ConnectorItem extends PolylineItem {
	 ...
	 *
	 * @param layer     layer to add the connector to (recommended: a connection
	 *                  layer below nodes)
	...
	public ConnectorItem(Layer layer, AItem startItem, AItem endItem) {
		...
		// Connections should usually be selectable (for elbow editing) but not
		// rotatable.
	...
	/**
	 * Recompute endpoint vertices (0 and last) from the items. Keeps elbow vertices
	 * unchanged.
	 */
	...
	/**
	 * Choose an anchor point on an item in world coordinates. For now: use the
	 * item's focus point. Later: intersect boundary in direction of other endpoint.
	 */
	...
	/**
	 * Initial L-shape: A -> (A.x, B.y) -> B. You can swap to (B.x, A.y) based on
	 * which looks cleaner.
	 */
	private static Point2D.Double[] initialPoints(IContainer c, AItem start, AItem end) {
		...
		return new Point2D.Double[] { new Point2D.Double(a.x, a.y), elbow, new Point2D.Double(b.x, b.y) };
	}
}
