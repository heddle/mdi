 ...
 * <ul>
 * <li>Keep a true drop-in API: existing subclasses should compile unchanged.</li>
 ...
 * <li>Improve persistence stability: use {@link #VIEWPROPNAME} (not frame title)
 * as the persistence key prefix.</li>
 * <li>Maintain backward compatibility for legacy persistence keys (including the
 * historical misspelling {@code "maxmized"}).</li>
 * </ul>
 *
 * <h2>FlatLaf note</h2>
 * <p>
 * Previous versions repainted the desktop on move/resize to work around look-and-feel
 * quirks. With FlatLaf this is typically unnecessary, so those "repaint quirks"
 * were removed; the component listener methods remain as no-ops for API continuity.
 * </p>
 */
...
public class BaseView extends JInternalFrame
     ...
     * <p>
     * IMPORTANT: persistence keys should not be derived from the window title because
     * titles change. This value is set from {@link PropertySupport#getPropName(Properties)}
     * when available, otherwise a reasonable fallback is used.
     * </p>
     */
    protected String VIEWPROPNAME = "?";

    /** Properties provided at construction time (and used as the view's configuration bag). */
    ...
    /**
     * Construct a BaseView from key/value pairs consumed by {@link PropertySupport#fromKeyValues(Object...)}.
     *
    ...
    public BaseView(Properties props) {
        ...
        // Use the configured propName if present; otherwise fall back to title; then class name.
     ...
     * <p>
     * Preference order: explicit propName property, then title, then simple class name.
     * </p>
     */
     ...
     * <p>
     * The container may be provided directly via properties, or via a container class
     * property, otherwise a {@link BaseContainer} is used.
     * </p>
     *
    ...
    /**
     * @return {@code true} if this view wraps its container in a {@link JScrollPane}.
     */
    ...
    /**
     * Optional hook for subclasses: called when a right click is detected.
     ...
     * <p>
     * This matches the legacy logic: if selected, true; otherwise checks the desktop's
     * visible frames list ordering.
     * </p>
     *
     ...
     * <p>
     * Some overview/minimap-style views keep a {@link VirtualWindowItem} that reflects
     * or controls the visible world window of this view.
     * </p>
     *
     ...
     * <p>
     * This method supports both the newer "dotted" keys (e.g. {@code prefix.visible})
     * and the legacy non-dotted keys (e.g. {@code prefixvisible}). It also supports
     * the historical misspelling {@code maxmized}.
     * </p>
     *
     ...
     * <p>
     * Uses {@link #getPropertyName()} as the stable prefix. Writes both the newer dotted keys
     * and a small set of legacy keys for backward compatibility.
     * </p>
     *
    ...
    /**
     * Convenience method to read a boolean property from this view's internal properties.
     *
    ...
    /**
     * With FlatLaf, we no longer force desktop repaints on move.
     * Kept as a no-op for API continuity.
     */
    @Override
    ...
    /**
     * With FlatLaf, we no longer force desktop repaints on resize.
     * Kept as a no-op for API continuity.
     */
    @Override
     ...
     * <p>
     * All reads from {@link PropertySupport} happen here, so the rest of the constructor
     * can act on a stable configuration object.
     * </p>
     */
    private static final class ViewInitConfig {
        ...
        static ViewInitConfig from(Properties props) {
            ...
            return new ViewInitConfig(title, propName,
                    standardDecorations, iconifiable, maximizable, resizable, closable,
                    scrollable, visible,
     ...
     * Kept as static fields to preserve legacy "stacking" behavior across created views.
     */
    ...
    /**
     * Applies frame-level configuration (decorations, close operation, title, etc.).
     */
    private static final class FrameConfigurer {
        ...
        static void apply(BaseView view, ViewInitConfig cfg) {
            ...
            // For container-backed views: location is applied here; bounds/pack handled later.
    ...
    private static final class ContainerFactory {
        ...
        static IContainer resolveContainer(Properties props, Rectangle2D.Double worldSystem) {
            ...
            // If a container class is specified, instantiate it using the expected constructor signature.
            Object containerClassObj = props.get(PropertySupport.CONTAINERCLASS);
            if (containerClassObj instanceof Class<?> rawClass
                    && IContainer.class.isAssignableFrom(rawClass)) {
    ...
    /**
     * Composes the Swing content for container-backed views:
     * margins, background, preferred size, scroll pane, optional split pane, optional toolbar.
     */
    ...
    /**
     * Installs key bindings for this view (currently delete/backspace -> delete selected items).
     */
        ...
        static void installDeleteBinding(BaseView view) {
            ...
            AbstractAction deleteAction = new AbstractAction() {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    // Preferred path: toolbar's delete if present (keeps behavior consistent with UI).
     ...
     * <p>
     * Uses the stable prefix {@link BaseView#getPropertyName()}.
     * Supports old key variants for backward compatibility.
     * </p>
     */
    private static final class ViewPersistence {
        ...
        static void applyToView(BaseView view, Properties props) {
            ...
            boolean vis = readBoolean(props,
                    dotted + "visible",
                    prefixBase + "visible",
                    false);

            boolean ontop = readBoolean(props,
                    dotted + "ontop",
                    prefixBase + "ontop",
                    false);

            // Support both correct and legacy misspelling
            boolean maximized = readBoolean(props,
                    dotted + "maximized",
                    dotted + "maxmized",
                    false);
            maximized = maximized || readBoolean(props,
                    prefixBase + "maximized",
                    prefixBase + "maxmized",
                    false);
        ...
        static Properties captureFromView(BaseView view) {
            ...
            // Backward-compat: legacy no-dot keys for the booleans that historically used them
