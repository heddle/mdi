 ...
 * (cells). Double-clicking a column in the {@code VirtualView} shifts all views
 * horizontally so that the chosen column becomes the visible "current" column.
 * <p>
 * Lifecycle note:
 * <ul>
 ...
 * {@link #addNotify()} (when this component is actually attached to a realized
...
public class VirtualView extends BaseView
	...
	/**
	 * Listener installed on the parent frame to keep the virtual desktop in sync
	 * with frame size.
	 */
	 ...
	 *
	 * @return the current virtual view instance, or {@code null} if none has been
	 *         created
	 */
	 ...
	 * <p>
	 * This is the correct place to safely resolve the top-level window ancestor and
	 * install a resize listener on the parent {@link JFrame}. Attempting to access
	 * the window ancestor in the constructor is unreliable.
	 */
	@Override
	public void addNotify() {
		...
		if (w instanceof JFrame) {
			...
			// Keep the virtual desktop's world sizing and offsets in sync with the real
			// frame size.
	 ...
	 * <p>
	 * The VirtualView is intended to be visually unobtrusive, so its title is
	...
	/**
	 * Reconfigure the virtual desktop world system and update all virtual window
	 * items.
	 * <p>
	 * This should be called when the parent application frame size changes. It
	 * updates the virtual desktop's world width to be {@code numCols * frameWidth}
	 * (one frame-width per virtual column) and updates the stored per-column
	 * offsets.
	 */
	 ...
	 * <p>
	 * Offsets are stored as points for historical reasons; only the X component is
	 ...
	 * <p>
	 * The view is created with a small fixed pixel size (thumbnail-style), and its
	 * world system is sized to {@code numCols * maxScreenWidth}.
	 * <p>
	 * The parent frame is resolved later in {@link #addNotify()} so it is safe to
	 * call this factory before the application frame is visible.
	 *
	 ...
	 * <p>
	 * The virtual desktop uses a single world width that is
	 * {@code numCols * maxMonitorWidth}. The parent frame size will later refine
	 * the effective visible column width via {@link #reconfigure()}.
	 */
	...
	/**
	 * Move a view to a specific column, applying extra offsets and a placement
	 * constraint.
	 *
	 * @param view       the view to move
	 * @param col        the target column
	...
	/**
	 * Activate the virtual column containing the given view so that it becomes
	 * visible.
	 * <p>
	 * This uses the view's {@link VirtualWindowItem} bounds inside the virtual
	 * desktop.
	 *
	 * @param view the view
	 */
