 ...
 * (Harbor/JRS properties) for component styling.</p>
 *
 * <p>The class is {@code final} and has a private constructor because it is
 * purely a static utility.</p>
 */
public final class WindowPlacement {
     ...
     *
     * <p>The method tries the following logic in order:</p>
     * <ol>
     *   <li>Return the bounds of the first screen whose {@code x} coordinate is
     *       {@code 0}. This is typically the primary monitor.</li>
     *   <li>If no such screen exists, return the bounds of the largest
     *       (by pixel area) display.</li>
     *   <li>If screen information is unavailable, fall back to the default
     *       Toolkit screen size.</li>
     * </ol>
     *
     * @return the rectangle representing the main screen's bounds; never {@code null}.
     */
     ...
     * {@link #centerComponent(Component, int, int)} with zero offsets.</p>
     *
     ...
     *
     * <p>The method ensures that the component does not exceed the screen size,
     * shrinking its calculated size if necessary, but it does not resize the
     * component itself—only its placement rectangle.</p>
     *
     ...
     * @param fractionalSize the fraction (e.g., {@code 0.8} for 80% of screen size).
     */
    ...
    /**
     * Returns the top-level Swing/AWT container associated with the given component.
     *
     * <p>The search walks up the parent hierarchy until it finds one of:</p>
     * <ul>
     ...
     * </ul>
     *
     * <p>If no such ancestor exists, or if {@code component} is {@code null}, the
