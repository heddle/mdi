...
//			maxw = Math.max(maxw, devices[i].getDisplayMode().getWidth());
	 ...
	 * @param azimuth
	 */
	public static void drawSnippets(Graphics g, int x, int y, Font font, String text, Component component,
			double azimuth) {
	...
	public static void saveAsPng(Component component) {
		try {
			...
			if (Environment.getInstance().getPngWriter() != null) {
				...
				file = FileUtils.saveFile(_dirname, "screencapture.png", "PNG ImageFile", "png", "PNG");
				...
				if (file != null) {
					...
					bi = GraphicsUtils.getComponentImage(component);
