 ...
 * <p>
 * This tool delegates interactive rubber-banding to {@link Rubberband}, which
 * in this codebase installs temporary mouse listeners directly on the
 * container's canvas component. When the gesture completes, {@link Rubberband}
 * calls back to {@link #doneRubberbanding()}.
 * </p>
 *
 * <h2>Event routing with the new tool framework</h2>
 * <p>
 * The toolbar/controller continues to receive mouse events, but during an
 * active rubber-band gesture, the {@link Rubberband}'s own listeners do the
 * real work. Consequently, this tool only needs to start the gesture on mouse
 * press and handle completion via the callback.
 * </p>
 *
 * <h2>Cancellation</h2>
 * <p>
 * If the tool is deselected while a rubber-band gesture is active, the tool
 * forces the rubber-band to terminate (so it can remove its temporary
 * listeners) but suppresses the zoom action.
 * </p>
 *
 * @author heddle
 */
public class BoxZoomTool implements ITool, IRubberbanded {
	 ...
	 * {@link IContainer#rubberBanded(Rectangle)}. If the gesture was canceled (tool
	 * deselected), the bounds are ignored.
	 * </p>
	 */
	@Override
	 ...
	 * <p>
	 * Since {@link Rubberband} installs listeners, we must ensure it terminates so
	 * it can remove them. We do this by calling
	 * {@link Rubberband#endRubberbanding(java.awt.Point)} using its current point,
	 * but we mark the termination as a cancellation so the zoom action is
	 * suppressed in {@link #doneRubberbanding()}.
	 * </p>
	 */
	@Override
	...
	/**
	 * Set the rubber-band policy used by this tool. Legal policies from
	 * {@link Rubberband.Policy} are
	 * <ul>
	...
	public void setPolicy(Rubberband.Policy policy) {
		...
		if ((policy == Rubberband.Policy.RECTANGLE) || (policy == Rubberband.Policy.RECTANGLE_PRESERVE_ASPECT)
				|| (policy == Rubberband.Policy.XONLY) || (policy == Rubberband.Policy.XONLY)) {
			this.policy = policy;
		} else {
