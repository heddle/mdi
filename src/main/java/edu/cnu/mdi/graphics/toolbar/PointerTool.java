 ...
 * <ul>
 * <li>Left-click on an unlocked item selects it (Ctrl enables additive
 * selection)</li>
 ...
 * <p>
 * When the press occurs on a trackable item, dragging begins a modification
 * gesture after a minimum movement threshold ({@link #MIN_DRAG_STEP}). Once
 * modifying, the tool updates the {@link ItemModification} current mouse point
 * and calls {@link AItem#modify()} on each drag event. On release, the
 * modification ends with {@link AItem#stopModification()}.
 * </p>
 *
 ...
 * {@link BaseToolBar} (or a dedicated popup router). This tool therefore does
 * not perform popup work; it simply ignores right-click presses.
 * </p>
 *
 * @author heddle
 */
public class PointerTool implements ITool, IRubberbanded {
	...
	public void mouseDragged(ToolContext ctx, MouseEvent e) {
		...
		if ((owner == null)
				|| (owner.getComponent() != null && !owner.getComponent().getBounds().contains(e.getPoint()))) {
			return;
		}
		...
		if (modifying) {
			...
			if (modifying) {
				ItemModification mod = new ItemModification(modifiedItem, owner, startPoint, currentPoint,
	...
	public void mouseReleased(ToolContext ctx, MouseEvent e) {
		try {
			// Rubberband completion is handled internally and will call
			// doneRubberbanding().
			if (rubberband != null) {
				return;
			}
	...
	/**
	 * When beginning a drag modification on a parent item, pre-create drag
	 * modifications for descendants so they move together.
	 *
	...
	private void prepareDescendantDrag(AItem item, MouseEvent e) {
		...
		for (AItem d : descendants) {
			...
			ItemModification dm = new ItemModification(d, owner, startPoint, currentPoint, e.isShiftDown(),
					e.isControlDown());
