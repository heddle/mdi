...
import edu.cnu.mdi.graphics.toolbar.button.DeleteButton;
import edu.cnu.mdi.graphics.toolbar.button.EditStyleButton;
import edu.cnu.mdi.graphics.toolbar.button.RefreshButton;
import edu.cnu.mdi.graphics.toolbar.button.ToolActionButton;
import edu.cnu.mdi.graphics.toolbar.button.ToolToggleButton;
import edu.cnu.mdi.graphics.toolbar.button.UndoZoomButton;
import edu.cnu.mdi.graphics.toolbar.button.WorldButton;
import edu.cnu.mdi.graphics.toolbar.button.ZoomInButton;
import edu.cnu.mdi.graphics.toolbar.button.ZoomOutButton;
import edu.cnu.mdi.graphics.toolbar.tool.BoxZoomTool;
import edu.cnu.mdi.graphics.toolbar.tool.CenterTool;
import edu.cnu.mdi.graphics.toolbar.tool.ConnectorTool;
import edu.cnu.mdi.graphics.toolbar.tool.EllipseTool;
import edu.cnu.mdi.graphics.toolbar.tool.ITool;
import edu.cnu.mdi.graphics.toolbar.tool.LineTool;
import edu.cnu.mdi.graphics.toolbar.tool.MagnifyTool;
import edu.cnu.mdi.graphics.toolbar.tool.PanTool;
import edu.cnu.mdi.graphics.toolbar.tool.PointerTool;
import edu.cnu.mdi.graphics.toolbar.tool.PolygonTool;
import edu.cnu.mdi.graphics.toolbar.tool.PolylineTool;
import edu.cnu.mdi.graphics.toolbar.tool.RadArcTool;
import edu.cnu.mdi.graphics.toolbar.tool.RectangleTool;
 ...
 * and routes mouse events from the container's canvas to the currently active
 * tool.
 * <p>
 * Popup triggers are handled centrally on both press and release (platform
 * dependent) using {@link PopupTriggerSupport}. Popup events are not forwarded
 * to tools.
 * </p>
 *
 * @author heddle
...
public class BaseToolBar extends CommonToolBar implements MouseListener, MouseMotionListener, IContainerToolBar {
	...
	/**
	 * Convenience reference for box zoom tool. It is cached so that the boxZoom
	 * rubberband policy can be changed from the default of
	 * Rubberband.Policy.RECTANGLE_PRESERVE_ASPECT
	 */
	...
	/**
	 * Attach mouse listeners to the container canvas so this toolbar can route
	 * events.
	 */
	...
	private void hookControllerSelectionToButtons() {
		toolController.addSelectionListener(activeTool -> {
			for (ToolToggleButton b : toolButtons.values()) {
				boolean shouldBeSelected = (activeTool != null) && b.toolId().equals(activeTool.id());
	 ...
	 * <p>
	 * This is the preferred way to trigger delete from keyboard shortcuts because
	 * it preserves the exact same behavior as the toolbar button (reset tool,
	...
	public void invokeDelete() {
		...
		// SwingUtilities.invokeLater(this::invokeDelete); return; }
