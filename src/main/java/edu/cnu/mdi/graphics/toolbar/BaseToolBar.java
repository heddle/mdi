 ...
 * </p>
 * <ul>
 *   <li><b>Mutually exclusive toggle tools</b> (pointer, pan, magnify, box zoom, drawing tools, connector, etc.)</li>
 *   <li><b>One-shot actions</b> (zoom in/out, undo/reset zoom, delete, style, camera, printer)</li>
 *   <li><b>Status field</b> (optional)</li>
 * </ul>
 *
 * <h2>GestureContext-first</h2>
 * <p>
 * All interactions are dispatched to {@link IToolHandler} using {@link GestureContext}.
 * Tools that originate from mouse gestures (press/drag/release, hover move, rubberband)
 * create/update a {@link GestureContext}. One-shot actions synthesize a context at the
 * canvas center with a {@code null} {@link MouseEvent}.
 * </p>
 *
 * <h2>Listener swapping</h2>
 * <p>
 * Only the active toggle tool receives mouse events from the canvas. This toolbar swaps
 * canvas listeners when the active toggle changes, tracking the currently installed
 * listeners directly for O(1) removal.
 * </p>
 *
 * <h2>Extensibility</h2>
 * <p>
 * Applications can add their own buttons after construction using the id-aware methods
 * from {@link AToolBar}:
 * </p>
 *
 * <pre>{@code
...
public class BaseToolBar extends AToolBar {
    ...
    /**
     * Creates a new horizontal toolbar associated with a canvas using common defaults:
     * pointer uses {@link ARubberband.Policy#RECTANGLE} and box zoom uses
     ...
     * <p>
     * The order is based on common usage patterns (selection/navigation first,
     * then drawing/annotation, then utilities).
     * </p>
     */
    private void addPredefinedButtons() {
            ...
            JToggleButton pointer = new APointerButton(canvas, this, pointerPolicy, DEFAULT_MIN_SIZE_PX) {
                ...
                public void rubberbanding(Rectangle bounds, Point[] vertices) {
                    GestureContext gc = new GestureContext(BaseToolBar.this, canvas, null,
                            (vertices != null && vertices.length > 0) ? vertices[0] : bounds.getLocation(),
                            null);
        ...
        if (ToolBits.hasLineButton(bits)) {
            rubberbandTool(ToolBits.LINE, ARubberband.Policy.LINE,
                    (gc, bounds, vertices) -> resetDefaultToggleButton());
        }
        ...
        if (ToolBits.hasPolygonButton(bits)) {
            rubberbandTool(ToolBits.POLYGON, ARubberband.Policy.POLYGON,
                    (gc, bounds, vertices) -> resetDefaultToggleButton());
        }

        if (ToolBits.hasPolylineButton(bits)) {
            rubberbandTool(ToolBits.POLYLINE, ARubberband.Policy.POLYLINE,
                    (gc, bounds, vertices) -> resetDefaultToggleButton());
        }
     ...
     * application-added tools (via {@link #addToggle(String, JToggleButton)}
     * and {@link #addButton(String, JButton)}).
     * </p>
     *
     ...
     * <p>
     * When enabled via {@link ToolBits#STATUS}, it will claim empty space at the end
     * of the toolbar via glue.
     * </p>
     *
