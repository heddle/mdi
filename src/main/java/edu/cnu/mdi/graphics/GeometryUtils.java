 ...
 *
 * <p>
 * These helpers are primitive building blocks used by view containers,
 * rubber-band selection, drawing tools, and interactive geometry components.
 * </p>
 *
 * <p>
...
public final class GeometryUtils {
	...
	/**
	 * Default tolerance (in pixels) for {@link #pointOnLine} hit testing. Values
	 * within this distance of the ideal line segment are considered a hit.
	 */
	 ...
	 * <p>
	 * This method automatically determines the upper-left corner and computes width
	 * and height using absolute differences.
	 *
	 * <p>
	 * Behavior with null inputs:
	 * </p>
	 * <ul>
	 * <li>If both {@code p1} and {@code p2} are {@code null}, returns
	 * {@code null}</li>
	 * <li>If exactly one point is {@code null}, returns a zero-sized rectangle at
	 * the other</li>
	 * </ul>
	 *
	 * @param p1 the first point; may be {@code null}
	 * @param p2 the second point; may be {@code null}
	 * @return a {@link Rectangle} spanning {@code p1} and {@code p2}, or
	 *         {@code null} if both points are {@code null}
	 */
	...
	/**
	 * Adjusts a drag endpoint so that the rectangle defined by <code>(p0, p)</code>
	 * matches the aspect ratio of the reference rectangle {@code r}. This is used
	 * in tools such as fixed-aspect zoom boxes or constrained shape creation.
	 *
	 * <p>
	 * The point {@code p} is modified in place.
	 * </p>
	 *
	 * <p>
	 * Aspect ratio rules:
	 ...
	 * <li>Sign of the drag is preserved so the rectangle grows in the correct
	 * direction</li>
	 * </ul>
	 *
	...
	/**
	 * Returns a new rectangle using the aspect-ratio-fixed adjustment applied to
	 * point {@code p}. This is a convenience wrapper that calls
	 * {@link #rectangleARFixedAdjust(Rectangle, Point, Point)} and then constructs
	 * a new rectangle from the two points.
	 *
	...
	/**
	 * Tests whether a point lies on or very near a finite line segment, using a
	 * default tolerance of {@link #DEFAULT_SELECT_RES}.
	 *
	 * <p>
	 * This is useful for hit-testing drawn line segments in GUI tools.
	 * </p>
	 *
	...
	/**
	 * Tests whether a point <code>(px,py)</code> lies on or near the line segment
	 * connecting <code>(sx,sy)</code> to <code>(ex,ey)</code>, within a specified
	 ...
	 *
	 * <p>
	 * This method is optimized for interactive drawing tools and uses simple
	 * arithmetic to avoid unnecessary divisions where possible.
	 * </p>
	 *
	 ...
	 * @return {@code true} if the test point is near the segment; {@code false}
	 *         otherwise
	 */
	public static boolean pointOnLine(int px, int py, int sx, int sy, int ex, int ey, double tolerance) {
